      subroutine sum_fluxes(iflux,jflux,delprop,prop_inc,frkut_sf)
...
! "prop" over the time step "deltat" and set the result to "delprop(i,j)"
      ...
      do i=1,ni-1
        do j=1,nj-1
          delprop(i,j) = frkut_sf*(deltat/area(i,j))*(iflux(i,j) - iflux(i+1,j) + jflux(i,j) - jflux(i,j+1))
	  store(i,j) = delprop(i,j)
	  delprop(i,j) = delprop(i,j) + facsec*(delprop(i,j) - previous(i,j))
        end do
      end do

! Residual Averaging
	call res_smooth(delprop,0.5,ni-1,nj-1)
