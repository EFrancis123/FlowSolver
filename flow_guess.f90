...
! Local stuff
      real ::   mflow, machlim, tlim, mach_num, Tdown,e,dy,dx,dxy, vel, dens
...
! INSERT your code here
	do i=1,ni
		aflow(i) = sqrt((y(i,nj)-y(i,1))**2 + (x(i,nj)-x(i,1))**2)
	end do
...
! INSERT your code here
	Tdown = tstagin * (pdown / pstagin)**((gamma - 1)/gamma)
	dens = pdown / (rgas * Tdown)
	vel = sqrt(2* cp * (tstagin - Tdown))
	mflow = dens * aflow(ni) * vel
	write(6,*) 'mflow: ', mflow
